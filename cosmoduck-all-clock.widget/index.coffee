# Cosmoduck · Tutto in uno, con orologio — Übersicht (legacy, lockable & draggable)
#
# La stessa card di cosmoduck-all.widget -- meteo, sistema, sensori, rete,
# processi, uso di Claude e calendario -- con l'orologio in cima. E' quello
# della card dedicata -- le stesse cifre a due strati con i colori invertiti,
# gli stessi secondi sotto -- ma le ore e i minuti stanno in riga invece che
# uno sopra l'altro: impilati com'e' li' vogliono centottanta punti d'altezza,
# e in cima a un pannello che ne misura gia' novecento sono troppi.
#
# E' una variante, non un'aggiunta: installala al posto di cosmoduck-all.widget,
# non insieme, o ti ritrovi lo stesso pannello due volte.
#
...
do ->
  try
    savedTop = localStorage.getItem('cosmoduck-all-clock_pos_top2')
    savedLeft = localStorage.getItem('cosmoduck-all-clock_pos_left2')
    return unless savedTop and savedLeft
    tries = 0
    place = ->
      el = document.querySelector('[id^="cosmoduck-all-clock-widget"]')
      if el
...
command: "bash cosmoduck-all-clock.widget/scripts/collect.sh"
...
style: """
  ...
  // Le cifre sono quelle della card dedicata, corpo 118 e spaziatura 2, dentro
  // un riquadro che non conserva le proporzioni (preserveAspectRatio="none",
  // come li'): la scala e' 0.756 in orizzontale e 0.84 in verticale, cioe' le
  // cifre restano allungate dello stesso undici per cento dell'originale.
  //
  // Quello che cambia e' la disposizione: i due blocchi stanno in riga, le ore
  // al margine sinistro come tutto il resto della card e i minuti a ventiquattro
  // unita' di distanza -- diciotto punti veri. Piu' stretti di cosi' si leggono
  // come un numero solo di quattro cifre, e l'ombra dei minuti va a toccare
  // l'inchiostro delle ore. I secondi vanno sotto, allineati a destra con i
  // minuti: l'ancora e' 2.46 unita' oltre il loro bordo perche' la S si porta
  // dietro la sua spalla piu' la spaziatura, e senza quel recupero la riga
  // finirebbe corta. A sinistra, sulla stessa riga, il giorno della settimana,
  // che parte 0.5 unita' prima delle ore perche' le lettere hanno una spalla
  // piu' stretta dello zero. Per esteso ci sta: WEDNESDAY, il piu' lungo, ne
  // occupa 95.5 delle 106 che restano prima dei secondi.
  //
  // Le 115 unita' d'altezza sono 85 di linea di base piu' le 26 che nella card
  // dedicata separano i minuti dai secondi, piu' quattro di margine: 96.6 punti
  // veri, contro i 180 che vorrebbero impilati.
  .clk
    display: block
    width: 100%
    height: 96.6px
...
render: -> """
  <style>
    @font-face{font-family:'CDAbel';src:url('cosmoduck-all-clock.widget/fonts/Abel-Regular.ttf') format('truetype');}
  ...
  <svg class="clk" viewBox="0 0 242.6 115" preserveAspectRatio="none">
    <text x="6"     y="85"  class="big" style="fill:var(--cd-accent, #5DADE2)" transform="translate(-4,4)" id="a-hh1">00</text>
    <text x="6"     y="85"  class="big" style="fill:var(--cd-shadow, #1F3A5F)" id="a-hh2">00</text>
    <text x="118.6" y="85"  class="big" style="fill:var(--cd-shadow, #1F3A5F)" transform="translate(-4,4)" id="a-mm1">00</text>
    <text x="118.6" y="85"  class="big" style="fill:var(--cd-accent, #5DADE2)" id="a-mm2">00</text>
    <text x="9.4"   y="111" class="sc" style="fill:var(--cd-bright, #AED6F1)" id="a-dow">&#8212;</text>
    <text x="213.6" y="111" class="sc" text-anchor="end"><tspan style="fill:var(--cd-bright, #AED6F1)" id="a-ss">00</tspan><tspan style="fill:var(--cd-text, #C8D9E8)"> SECONDS</tspan></text>
  </svg>
...
afterRender: (domEl) ->
  ...
  # ── orologio ───────────────────────────────────────────────────────────────
  # I secondi non possono venire dal comando: quello del pannello dura circa un
  # secondo intero -- il collector di rete guarda i contatori per un secondo per
  # sapere a che velocita' vanno -- e gira ogni cinque. L'ora la tiene il browser,
  # che legge lo stesso orologio di sistema, e si ridisegna da sola.
  #
  # Non con un setInterval da mille millisecondi: quello accumula ritardo, e
  # dopo un po' il numero cambia a meta' del secondo di sistema mentre l'orologio
  # del menu cambia per conto suo. Ogni giro si riaggancia al bordo del secondo
  # che viene, con dieci millisecondi di margine per non arrivare un filo prima
  # e ripetere due volte lo stesso numero.
  #
  # `pad` e' quella del calendario, qui sopra.
  WEEKDAYS = ['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY',
              'SATURDAY']
  clock = ->
    now = new Date()
    # Riscritto a ogni giro come l'ora: cosi' a mezzanotte cambia da solo.
    $(domEl).find('#a-dow').text(WEEKDAYS[now.getDay()])
    $(domEl).find('#a-hh1,#a-hh2').text(pad(now.getHours()))
    $(domEl).find('#a-mm1,#a-mm2').text(pad(now.getMinutes()))
    $(domEl).find('#a-ss').text(pad(now.getSeconds()))
    domEl.__clock = setTimeout(clock, 1010 - now.getMilliseconds())
  # Se Übersicht ricostruisce la card senza buttare via l'elemento, il battito
  # di prima resterebbe appeso e ne avremmo due.
  clearTimeout(domEl.__clock) if domEl.__clock
  clock()
